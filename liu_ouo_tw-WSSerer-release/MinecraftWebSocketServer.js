...
class MinecraftWebSocketServer extends EventEmitter {
  constructor(port, aiWakeWord, playerRegex, minecraftAI, cooldown = 5, openweatherApiKey, geocodingApiKey, musicDir, commandsDir, mathDbPath, earthquakeApiKey, wolframApiKey, statsPath, ownApiKey, customSettings = {}) {
    ...
    this.discordToken = customSettings.discordToken || "";
    this.discordChannelId = customSettings.discordChannelId || "";
  ...
  async playerMessage(sender, message) {
    ...
    const isAnyWakeWord = 
      message.includes(this.aiWakeWord) ||
      message.includes(this.weatherWakeWord) ||
      message.includes(this.weatherPredictsWakeWord) ||
      message.includes(this.artWakeWord) ||
      message.includes(this.musicWakeWord) ||
      message.includes(this.nextWakeWord) ||
      message.includes(this.stopWakeWord) ||
      message.includes(this.playlistWakeWord) ||
      message.includes(this.examWakeWord) ||
      message.includes(this.answerWakeWord) ||
      message.includes(this.helpWakeWord) ||
      message.includes(this.cpuWakeWord) ||
      message.includes(this.cpuStopWakeWord) ||
      message.includes(this.earthquakeWakeWord) ||
      message.includes(this.calculaterWakeWord) ||
      message.includes(this.mazeWakeWord) ||
      message.includes(this.playlistEditWakeWord);

    if (!isAnyWakeWord) return;
  ...
  sendPrivateMessage(player, msg) {
    if (player && player.includes("Discord")) {
      this.emit("broadcastMessage", msg);
  ...
  sendSplitPrivateMessage(player, message){
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (estimateFinalPayloadBytes(testChunk) <= 661) {
        currentChunk.push(line);
      } else {
        if (currentChunk.length > 0) {
          const escapedMsg = JSON.stringify(currentChunk.join("\n"));
          this.runCommand(`tellraw "${player}" {"rawtext":[{"text":${escapedMsg}}]}`);
        }
        currentChunk = [line];
      }
