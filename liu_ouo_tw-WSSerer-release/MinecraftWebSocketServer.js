...
class MinecraftWebSocketServer extends EventEmitter {
  ...
  async playerMessage(sender, message) {
    ...
    const weatherPredictsRegex = new RegExp(`^${escapeRegExp(this.weatherPredictsWakeWord)}\\s*(.+)$`);
    ...
    match = message.match(weatherPredictsRegex);
    if (match){
      const location = match[1].trim();
      this.handleWeatherForecast(sender, location);
      return;
    }
  ...
  async handleWeatherForecast(sender, location){
    if (!this.openweatherApiKey || !this.geocodingApiKey){
      this.sendPrivateMessage(sender, "§c[系統] 伺服器未設定 Weather 或 Geocoding API Key，無法使用預報功能");
      return;
    }
    try{
      const geoUrl = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(location)}&language=zh-TW&key=${this.geocodingApiKey}`;
      const geoRes = await axios.get(geoUrl);
      if (!geoRes.data || geoRes.data.status !== "OK" || geoRes.data.results.length === 0){
        this.sendChat(sender, `§c[系統] 找不到地點：「${location}」，請檢查地名是否正確。`);
        return;
      }
      const { lat, lng: lon } = geoRes.data.results[0].geometry.location;
      const cityName = geoRes.data.results[0].formatted_address;

      const currentWeatherUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&lang=zh_tw&appid=${this.openweatherApiKey}`;
      ...
      const forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=metric&lang=zh_tw&appid=${this.openweatherApiKey}`;
      const forecastRes = await axios.get(forecastUrl);
      if (!forecastRes.data || !forecastRes.data.list){
        this.sendChat(sender, "§c[系統] 天氣預報資料獲取失敗");
        return;
      }
      ...
      forecastRes.data.list.forEach(item => {
        ...
        // 6. ~ 9.
        if (hour >= 6 && hour <= 9 && !dailyData[dateStr].morning) {
          dailyData[dateStr].morning = item;
        }
        // 12. ~ 3. PM
        else if (hour >= 12 && hour <= 15 && !dailyData[dateStr].afternoon) {
          dailyData[dateStr].afternoon = item;
        }
        // 6. ~ 9. PM
        else if (hour >= 18 && hour <= 21 && !dailyData[dateStr].evening) {
          dailyData[dateStr].evening = item;
        }
      });
      ...
      this.sendPrivateMessage(sender, `§b${cityName} 的天氣預報`);
      this.sendPrivateMessage(sender, `§f• 目前體感溫度：${currentFeelsLike}°C (${currentDesc})`);
      this.sendPrivateMessage(sender, `§f• 目前濕度：${currentHumidity}%`);
      this.sendPrivateMessage(sender, `§7-------------------------------------`);
      this.sendPrivateMessage(sender, `§b未來幾天預報：`);

      const sortedDates = Object.keys(dailyData).sort().slice(0, 5);
      sortedDates.forEach(date => {
        const slots = dailyData[date];
        this.sendPrivateMessage(sender, `§e• ${date}：`);
        this.sendPrivateMessage(sender, `  §6早上：${formatSlot(slots.morning)}`);
        this.sendPrivateMessage(sender, `  §g中午：${formatSlot(slots.afternoon)}`);
        this.sendPrivateMessage(sender, `  §9晚上：${formatSlot(slots.evening)}`);
      });
    } catch (error) {
      this.emit("log", `查詢詳細預報失敗: ${error.message}`);
      this.sendPrivateMessage(sender, `§c[系統] 讀取天氣預報時發生非預期錯誤`);
    }
  }
