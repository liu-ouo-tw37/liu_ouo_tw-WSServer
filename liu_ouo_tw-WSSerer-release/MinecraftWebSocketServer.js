...
class MinecraftWebSocketServer extends EventEmitter {
  constructor(port, aiWakeWord, playerRegex, minecraftAI, cooldown = 5, openweatherApiKey, geocodingApiKey, musicDir, commandsDir, mathDbPath, earthquakeApiKey, wolframApiKey, statsPath) {
  ...
  async playerMessage(sender, message) {
    ...
    if (this.playlistEditWakeWord && message.includes(this.playlistEditWakeWord)){
      if (!isAdmin) return;

      const parts = message.replace("-playlist_edit ", "").trim().split(" ");
      const action = parts[0];
      const listName = parts[1];
      const songName = parts.slice(2).join(" ");
