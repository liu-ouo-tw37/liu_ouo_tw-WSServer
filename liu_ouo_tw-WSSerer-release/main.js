...
const initWeatherPlugin = require("./plugins/weather");
const initExamPlugin = require("./plugins/exam");
const initMusicPlugin = require("./plugins/music");
const initMazePlugin = require("./plugins/maze");
const initEarthquakePlugin = require("./plugins/earthquake");
const initCpuPlugin = require("./plugins/cpu");
const initCalcPlugin = require("./plugins/calc");
const initDiscordPlugin = require("./plugins/discord");
...
let settings = {
  ...
  discordChannelId: "",
  wakeWord: "-ai?",
  weatherWakeWord: "-weather?",
  weatherPredictsWakeWord: "-weather_predicts?",
  artWakeWord: "-art?",
  musicWakeWord: "-music?",
  examWakeWord: "-exam?",
  answerWakeWord: "-answer?",
  calculaterWakeWord: "-calc?",
  playerRegex: "",
  maxOutput: 512,
  creativity: 1,
...
async function startWebSocketServer(settings) {
  ...
  const statsPath = path.join(resourcePath, "players_stats.json");
  ...
  if (settings.enableAI !== false) {
    ...
    writeLog("ℹ️ 系統設定已關閉 AI 功能，跳過 AI 連線測試。");
  }
  ...
  initWeatherPlugin(mcWSS);
  initExamPlugin(mcWSS);
  initMusicPlugin(mcWSS);
  initMazePlugin(mcWSS);
  initEarthquakePlugin(mcWSS);
  initCpuPlugin(mcWSS);
  initCalcPlugin(mcWSS);
  initDiscordPlugin(mcWSS);
