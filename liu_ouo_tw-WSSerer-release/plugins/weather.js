...
function initWeatherPlugin(server){
    ...
    async function handleWeatherForecast(sender, location) {
        ...
        server.sendPrivateMessage(sender, `§b${cityName} 的天氣預報`);
        server.sendPrivateMessage(sender, `§f• 目前體感溫度：${currentFeelsLike}°C (${currentDesc})`);
        server.sendPrivateMessage(sender, `§f• 目前濕度：${currentHumidity}%`);
        server.sendPrivateMessage(sender, `§7-------------------------------------`);
        server.sendPrivateMessage(sender, `§b未來幾天預報：`);

        const sortedDates = Object.keys(dailyData).sort().slice(0, 5);
        sortedDates.forEach(date => {
            const slots = dailyData[date];
            server.sendPrivateMessage(sender, `§e• ${date}：`);
            server.sendPrivateMessage(sender, `  §6早上：${formatSlot(slots.morning)}`);
            server.sendPrivateMessage(sender, `  §g中午：${formatSlot(slots.afternoon)}`);
            server.sendPrivateMessage(sender, `  §9晚上：${formatSlot(slots.evening)}`);
        });
        } catch (error) {
